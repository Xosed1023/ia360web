...
function HybridSphere() {
  ...
  const { particles, originalPositions } = useMemo(() => {
    const positions = []
    const originals = []
    const particleCount = 2000 // Más partículas que NetworkSphere

    for (let i = 0; i < particleCount; i++) {
      const theta = Math.random() * Math.PI * 2
      const phi = Math.acos(2 * Math.random() - 1)
      const radius = 2.2 + Math.random() * 0.8 // Variación en el radio
  ...
  const { lines, lineIndices } = useMemo(() => {
    const linePositions = []
    const indices = []
    const maxDistance = 0.3 // Conexiones más cercanas para más detalle
  ...
  useFrame(() => {
    ...
    if (pointsRef.current) {
      ...
      for (let i = 0; i < positions.length; i += 3) {
        ...
        // Cambio de color gradual (transición entre teal, cyan y azul)
        const colorShift = Math.sin(time.current * 0.5 + i * 0.01) * 0.5 + 0.5
        const color = new THREE.Color()
        color.lerpColors(
          new THREE.Color('#14b8a6'), // Teal
          new THREE.Color('#9be585'), // Cyan
          colorShift
        )
        const finalColor = new THREE.Color()
        finalColor.lerpColors(
          color,
          new THREE.Color('#344b47'), // Cyan dark
          Math.sin(time.current * 0.3 + i * 0.02) * 0.5 + 0.5
        )
